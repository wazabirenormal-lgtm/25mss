...
# Reemplazado por variable de entorno
...
class DarkluaConfigView(View):
    def __init__(self, user_id: int, filename: str):
        ...
        self.available_rules = [
            "compute_expression", "remove_unused_while", "remove_unused_if_branch",
            "remove_nil_declaration", "convert_index_to_field", "remove_comments",
            "remove_method_definition", "remove_spaces", "remove_types",
            "remove_unused_variable", "remove_function_call_parens", "remove_empty_do",
            "remove_compound_assignment", "remove_continue", "remove_if_expression",
            "remove_interpolated_string", "remove_floor_division", "filter_after_early_return",
            "group_local_assignment", "rename_variables",
        ]
    ...
    if not result or not filename:
        ...
        try:
            await applydarklua('./dumps/dumped/' + filename, [
                "compute_expression", "remove_unused_while", "remove_unused_if_branch",
                "remove_nil_declaration", "convert_index_to_field"
            ])
...
async def beautify_cmd(msg):
    ...
    if filename:
        if await luabeautify(f"./dumps/beautify/{filename}", ["compute_expression"]):
...
async def minify_cmd(msg):
    ...
    if filename:
        if await applydarklua(f"./dumps/beautify/{filename}", [
            "convert_index_to_field", "compute_expression", "filter_after_early_return",
            "group_local_assignment", "remove_comments", "remove_method_definition",
            "remove_nil_declaration", "remove_spaces", "remove_types",
            "remove_unused_if_branch", "remove_unused_variable", "remove_unused_while",
...
async def cmds_access_cmd(msg):
    ...
    if "clan" in user_profile and user_profile["clan"] and "tag" in user_profile["clan"] and user_profile["clan"]["tag"] == "25ms":
        ...
        if roles and not any(role.id == 1385300853526892584 for role in roles):
            ...
            await softerror(msg, "You already have the 25ms server tag role!")
    else:
...
async def moonveil_obfuscate(msg):
    ...
    payload = {
        "options": {
            "cffDecomposeExpr": False, "cffEnable": True, "cffHoistLocals": True,
            "cffWrapBlocks": True, "mangleEnable": True, "mangleGlobals": True,
            "mangleNamedIndex": True, "mangleNumbers": False, "mangleSelfCalls": True,
            "mangleStrings": True, "prettify": False, "removeCompoundAssign": True,
...
async def goofy_fus(msg):
    ...
    response = await bypass.asyncpost("https://goofyscator.lua.cz/obfuscate", {
        "source": content,
        "settings": {
            "dontModifyBytecode": True, "hexNumbers": True, "renameGlobals": False,
            "generator": "Shuffled"
        }
    })
...
async def pastefy_upload(content):
    response = await bypass.asyncpost("https://pastefy.app/api/v2/paste", {
        "content": content, "title": "uploaded by 25ms", "encrypted": False,
        "visibility": "UNLISTED", "type": "PASTE", "tags": [], "ai": True
    })
...
async def debian_upload(content):
    ...
    res = await bypass.asyncpost(
        "https://paste.debian.net/api/v1/paste", {
            ...
            "poster": "by 25ms", "lang": "luau", "expire": 0
        },
...
uploaders = {
    "pastebin": pastebin_upload, "rubis": rubis_upload,
    "pastefy": pastefy_upload, "debian": debian_upload,
}
...
command_manager.commands = {
    ".obf": {"func": nonfunc, "description": "Obfuscate your lua code. Options: weak"},
    ".moonveil": {"func": moonveil_obfuscate, "description": "Get a free daily obfuscation using moonveil.cc", "cooldown": 10},
    ".goofy": {"func": goofy_fus, "description": "Free obfuscation using goofyscator by 1xayd1", "cooldown": 10},
    ".get": {"func": get_cmd, "description": "Fetch content from a link!", "cooldown": 10},
    ".upload": {"func": upload_cmd, "description": "Upload a file! Options: pastebin, rubis, debian, pastefy (default: pastefy)", "cooldown": 10},
    ".msdeobf": {"func": msdeobf, "description": "Moonsec deobfuscation, can be used for free in #moonsec-deobf. Decompiled with medal fork!", "cooldown": 10, "roles": [1373857675497963601], "allow_channels": [1348000639753519205]},
    ".detect": {"func": detect_cmd, "description": "Detect the obfuscator that a script is using", "cooldown": 3},
    ".ibs": {"func": nonfunc, "description": "ib2 based obfuscation. Probably more secure than moonsec, creates big files"},
    ".beautify": {"func": beautify_cmd, "description": "Beautify lua code."},
    ".darklua": {"func": darklua_gui_cmd, "description": "darklua process panel", "cooldown": 5},
    ".help": {"func": help_command, "description": "Displays help message"},
    ".ironobf": {"func": nonfunc, "description": "Obfuscation around the level of luaobfuscator"},
    ".ib2": {"func": nonfunc, "description": "Pretty bad obfuscation but the most performant one, also has very good compatibility. Good for huge scripts"},
    ".deobf": {"func": luaobf_deobf, "description": "Decrypt strings in string encrypted luaobfuscator scripts"},
    ".promdeobf": {"func": prom_deobf_api_cmd, "description": "Deobfuscate prometheus files using RELUA", "cooldown": 10},
    ".ibdeobf": {"func": ib2_deobf, "description": "Deobfuscate ironbrew 2 scripts.", "roles": [1373857675497963601], "allow_channels": [1348000639753519205]},
    ".minify": {"func": minify_cmd, "description": "minify your script"},
    ".compress": {"func": compress_cmd, "description": "compress your script"},
    ".solara": {"func": solara_cmd, "description": "get the download + version info to solara", "cooldown": 15},
    "meow": {"func": meow_cmd, "description": "meows", "cooldown": .1},
    ".decompile": {"func": decompile_manager, "description": "Decompile BYTECODE files (NOT SCRIPTS). Options: roblox, lua (default: lua -> medal fork) Usage: .decompile <option>", "cooldown": 5},
    ".oraclekey": {"func": oracle_key_cmd, "description": "Set your oracle key, usage: .oraclekey <key|remove>", "cooldown": 5},
    ".claim": {"func": claim_license, "description": "Claim a license key for the dumper :>", "cooldown": 15},
    ".gen": {"func": gen_cmd, "description": "generate an image using ai", "cooldown": 10, "roles": [1308178268821651456]},
    ".lconfig": {"func": dumpConfig_cmd, "description": "configure settings for .l", "cooldown": 45, "roles": [1373857675497963601]},
    ".getrecovery": {"func": get_recovery_cmd, "description": "View your recovery key", "cooldown": 8, "roles": [1373857675497963601]},
    ".rename": {"func": rename_cmd, "description": "rename lua!", "cooldown": 5, "roles": [1373857675497963601]},
    ".protect": {"func": protect_webhook_cmd, "description": "Best cheap webhook protection! Usage: .protect <webhook_url>", "cooldown": 5, "roles": [1373857675497963601]}
}
...
webhook_pres = [
    ...
    "https://webhook.lewisakura.moe/api/webhooks/", "https://stealer.to/",
    "https://discord-stealer.de/", "https://webhook.lewisakura.moe",
    "https://webhook-protect.vercel.app/", "https://dcwh.my/",
    "https://webhook-protector-worker.sharkyscripts.workers.dev/",
    "https://sharky-on-top.script-config-protector.workers.dev/",
    "https://webhook-protect-2.vercel.app/", "https://proxy-phi-nine-86.vercel.app/send",
    "https://webhook.whitehill.group/api/webhooks/", "https://rbxhook.cc/r/"
]
...
def send_webhawk(url, content=None):
    ...
    asyncio.create_task(send_discord_webhook(url, sendtxt, content))
...
# Reemplazado token de autorización en el header por variable de entorno
headers = {
    ...
    "x-super-properties": ""
}
...
async def getlinkcontent(text):
    ...
    if not url:
        return
    urls = [
        "[https://pastebin.com/](https://pastebin.com/)", "[https://raw.githubusercontent.com/](https://raw.githubusercontent.com/)",
        "[https://gist.githubusercontent.com/](https://gist.githubusercontent.com/)", "[https://pastefy.app/](https://pastefy.app/)",
        "[https://paste.ee/r/](https://paste.ee/r/)", "[https://rawscripts.net/raw/](https://rawscripts.net/raw/)",
        "[https://scriptblox.com/script/](https://scriptblox.com/script/)", "[https://pandadevelopment.net/virtual/file/](https://pandadevelopment.net/virtual/file/)"
    ]
    urls_regex = [r"[https://github.com/](https://github.com/)[A-z0-9_.-]+/[A-z0-9_.-]+/raw/"]
    if any(url.startswith(url_match) for url_match in urls) or any(re.match(urls_regex_match, url) for urls_regex_match in urls_regex):
        try:
            return (await asyncget(
                ...
                headers={"User-Agent": "Roblox/WinInetRobloxApp/0.673.0.6730711 (GlobalDist; RobloxDirectDownload)"}
...
async def obfhandler(msg, addCG=False):
    ...
    await applydarklua("unobfuscated/" + randomfilename, [
        "remove_empty_do", "remove_compound_assignment", "remove_continue",
        "remove_types", "remove_interpolated_string", "remove_if_expression",
    ...
    if "Writing output" in process.stdout:
        if obfmode == "Normal":
            await applydarklua(f"./obfuscated/{randomfilename}", [
                "convert_index_to_field", "group_local_assignment", "remove_method_definition",
                "remove_nil_declaration", "remove_spaces", "remove_function_call_parens",
